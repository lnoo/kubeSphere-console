...
export default class Tracing extends React.Component {
  constructor(props) {
    ...
    const selectCluster =
      clusters.find(item => get(item, 'configz.servicemesh')) || clusters[0] || {};
    ...
    if (selector) {
      ...
      this.serviceStore.fetchListByK8s(params).then(() => {
        ...
        if (components.length > 0) {
          this.setState({ serviceName: components[0].name }, () => this.fetchTracing());
        }
      });
    }
  }
  ...
  serviceRenderer = option => <span>{t('SERVICE_VALUE', { value: option.label })}</span>;
  ...
  renderOperations() {
    ...
    return (
      <div className={styles.operations}>
        ...
        />
        <Select value={query.limit} options={this.limitOptions} onChange={this.handleLimitChange} />
