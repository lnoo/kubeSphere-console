...
export default class Components extends Component {
  ...
  getNodePorts(gateway) {
    ...
    return gateway.ports.map(port => `${port.name.toUpperCase()}: ${port.nodePort}`).join('/');
  }
      ...
      <Panel>
        <div className={styles.header}>
          <div className={styles.cluster}>
            <ClusterTitle cluster={clusters[cluster]} theme="light" tagClass="float-right" />
          </div>
          <Text icon="eip-pool" title={gateway.type} description={t('GATEWAY_ACCESS_MODE')} />
          {gateway.type === 'NodePort' ? (
            <>
              ...
              />
              <Text title={this.getNodePorts(gateway)} description={t('NODE_PORTS_SCAP')} />
            </>
          ) : (
            <Text title={this.getExternalIP(gateway)} description={t('LOAD_BALANCER_SCAP')} />
          )}
        </div>
        <div className={styles.content}>
          <Loading spinning={isLoading}>
            <>
              {data.map(item => (
                <IngressCard key={item.name} detail={item} prefix={this.prefix} gateway={gateway} />
              ))}
            </>
          </Loading>
        </div>
