...
import { isEmpty, flatten, uniqBy, keyBy, throttle, get, cloneDeep } from 'lodash';
...
class ResourceStatus extends React.Component {
  ...
  renderReplicaInfo() {
    ...
    const clustersDetail = keyBy(this.props.projectStore.detail.clusters, 'name');
  ...
  renderContainerPorts() {
    ...
    return <ContainerPortsCard ports={ports} loading={isResourcesLoading} isFederated />;
  }

  renderPods() {
    const { detail, resources } = this.store;
    const clustersMap = keyBy(this.props.projectStore.detail.clusters, 'name');
    const clusters = detail.clusters.filter(item => clustersMap[item.name]).map(item => item.name);
