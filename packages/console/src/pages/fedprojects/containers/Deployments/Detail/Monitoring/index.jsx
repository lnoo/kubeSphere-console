...
export default class Monitorings extends React.Component {
  ...
  get isMore() {
    const data =
      get(toJS(this.resourceStore.sort), `data[${MetricTypes.cpu_usage}].data.result`) || [];
  ...
  renderCard() {
    ...
    return (
      <MonitoringController
        ...
        isFederated
      >
        {isMore && <Alert message={t.html('MONITORING_ALERT_DESC')} type="info" />}
        {configs.map(item => {
          item.data = get(this.metrics, `${item.metricType}.data.result`) || [];
          item.legend = item.data.map((record, index) => get(record, 'metric.pod', `pod${index}`));
          ...
          return (
            <div key={item.title} className={classnames(styles.item)}>
              {isMore && (
                <div className={styles.more} onClick={this.showMultipleModal(item)}>
                  {t('VIEW_ALL_REPLICAS')}
                </div>
              )}
