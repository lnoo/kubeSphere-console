...
export default class VolumeDetail extends React.Component {
  ...
  fetchData = async () => {
    ...
    const clusters = this.props.projectStore.detail.clusters.map(item => item.name);
  ...
  getAttrs = () => {
    ...
    return [
      ...
      },
      {
        ...
        value: storageClassName || annotations['volume.beta.kubernetes.io/storage-class'],
      },
      {
        name: t('PROVISIONER'),
        value: annotations['volume.beta.kubernetes.io/storage-provisioner'] || '-',
      },
      {
  ...
  render() {
    ...
    return <DetailPage stores={stores} {...sideProps} routes={getRoutes(this.props.match.path)} />;
  }
}
