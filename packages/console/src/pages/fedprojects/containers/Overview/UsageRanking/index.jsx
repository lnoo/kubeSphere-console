    ...
    return (
      <div className={styles.content}>
        {data.slice(0, 5).map(app => {
          ...
          const percent = (app[this.store.sort_metric] * 100) / data[0][this.store.sort_metric];

          const unit = getSuitableUnit(app[this.store.sort_metric], UNITS[this.store.sort_metric]);

          return (
            <div className={styles.app} key={`${app.owner_kind}-${workloadName}`}>
              <div className={styles.appContent}>
                <Icon name={ICON_MAP[app.owner_kind] || ICON_MAP.Default} type="dark" size={40} />
                <div className={styles.text}>
                  <div className="relative" data-tooltip={workloadName}>
                    <div>{link ? <Link to={link}>{workloadName}</Link> : workloadName}</div>
                  </div>
