...
export default class ScheduleCluster extends Component {
  ...
  get getWeight() {
    ...
    const weight = get(store.deployedScheduleTemplate, `spec.clusters.${cluster.name}.weight`, 0);
    return weight;
  }
  ...
  render() {
    ...
    const { status, reason } = getWorkloadStatus(workload, store.resourceStore.module);
    ...
    return (
      <div className={classNames(styles.scheduleCluster)}>
          ...
          <div className={styles.pods}>
            {Array(Math.min(podNums, 6))
              ...
              .map((_, index) => (
                <img
                  key={index}
                  src={index < unReadyNums ? '/assets/pod-waiting.svg' : '/assets/pod-ready.svg'}
                  alt=""
                />
              ))}
            {podNums > 6 && (
        ...
        <div className={styles.weight} onClick={this.stopPropagation}>
          <Icon name="add" type="light" size={20} clickable onClick={this.handleAdd} />
          <span className={styles.value}>
            <span className={styles.spanText}>{this.getWeight}</span>
            <span className={styles.subText}> {t('WEIGHT')}</span>
          </span>
          <Icon name="substract" type="light" size={20} clickable onClick={this.handleSubStract} />
        </div>
      </div>
    );
  }
