...
export default class ClusterWorkloadStatus extends Component {
  getWeight = name => {
    ...
    const weight = get(store.deployedScheduleTemplate, `spec.clusters.${name}.weight`, 0);
    return weight;
  };
  ...
  handleReplicasChange = async (cluster, replicas) => {
    ...
    if (override) {
      const path = override.clusterOverrides.find(item => item.path === '/spec/replicas');
      if (path) {
        path.value = replicas;
      } else {
    ...
    await store.resourceStore.patch({ ...detail, cluster }, { spec: { replicas } });
  };
  ...
  handleWeightChange = async (cluster, weight) => {
    ...
    set(store.deployedScheduleTemplate, `spec.clusters.${cluster}.weight`, weight);
  ...
  render() {
    const { store, resources, clusters, clustersDetail, canEdit, projectStore } = this.props;
