...
export default class ClusterSelect extends Component {
  ...
  handleDOMClick = e => {
    if (this.optionsRef && this.optionsRef.current && !this.optionsRef.current.contains(e.target)) {
  ...
  renderValue(cluster) {
    ...
    return (
      <div key={cluster.name} className={styles.option} onClick={this.toggleOptions}>
        <Icon
  ...
  renderOption(cluster) {
    const { extra } = this.props;
    return (
      <div key={cluster.name} className={styles.option} onClick={() => this.handleClick(cluster)}>
        <Icon
          name={CLUSTER_PROVIDER_ICON[cluster.provider] || 'kubernetes'}
          size={20}
          type="light"
