...
export default class AdvancedSettings extends React.Component {
  ...
  get defaultClusters() {
    return get(this.formTemplate, 'spec.placement.clusters', []).map(item => item.name);
  }
  ...
  handleLocal = clusters => {
    ...
    overrides.map(item => {
      if (!clusters.includes(item.clusterName) && !isEmpty(item.clusterOverrides)) {
  ...
  render() {
    const { formRef, projectDetail, withService, formProps, module, isEdit, workspace } =
      this.props;
        ...
        <Form data={this.formTemplate} ref={formRef} {...formProps}>
          ...
          {module === 'service' && isEdit ? null : (
            ...
            >
              ...
              >
                {props => (
                  <ContainersMapper formTemplate={this.formTemplate} {...props} {...formProps}>
                    {containerProps => (
                      <ContainerImage isEdit={isEdit} {...containerProps} workspace={workspace} />
                    )}
                  </ContainersMapper>
                )}
              </ClustersMapper>
              ...
              {module === 'service' && isEdit
                ? props => (
                    <ContainersMapperWithService
                      ...
                      {...formProps}
                    >
                      {containerProps => <ContainerPortsWithService {...containerProps} />}
                    </ContainersMapperWithService>
                  )
                : props => (
                    <ContainersMapper
